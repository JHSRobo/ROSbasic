...
from crack_finder.srv import crack_finder
...
def find_the_shape():
    try:
        ...
        while True:
        	...
        	resized = imutils.resize(cv_image, width=640)
        			...
        			for c in cnts:
        				...
        				if w>0 and h>0:
        					if shape == "rectangle":
        						...
        						cv2.imwrite("foundthecrack.png", cv_image)
        						image4 = cv_image
...
def listener():
    ...
    rospy.Service('crack_finder', shape_detect, find_the_shape)
    rospy.spin()
